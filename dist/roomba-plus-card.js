function re(t,n,e,o,s){let r=d=>!!t.states[`sensor.${n}_${d}`],a=d=>!!t.states[`select.${n}_${d}`],i=d=>!!t.states[`binary_sensor.${n}_${d}`],l=d=>!!t.states[`image.${n}_${d}`],c=r("mop_pad"),p=r("brush_remaining_hours");return{hasArea:r("area_cleaned_today"),hasBrush:p,hasPad:c,hasWater:r("mop_tank_level"),hasCleanBase:r("clean_base_status"),hasZones:a("smart_zone_select")||a("zone_select"),hasSmartZones:a("smart_zone_select"),hasProblemZone:r("problem_zone"),hasLifetimeArea:r("lifetime_area"),hasWearRate:r("filter_wear_rate"),isMop:c&&!p,hasMissionActive:i("mission_active"),hasMissionPhase:r("mission_phase"),hasDemandBlocked:i("demand_clean_blocked"),hasEnergyConsumption:r("total_energy_consumed"),hasCleaningSpeedTrend:r("cleaning_speed_trend"),hasBatteryRetention:r("battery_capacity_retention"),hasWifiFloor:r("recent_wifi_floor"),hasCoveragePct:r("recent_coverage_pct"),hasBatteryEol:r("estimated_battery_eol"),hasConsecutiveSkips:i("consecutive_clean_skips"),hasMopBehavior:r("mop_behavior"),hasCoverageImage:l("coverage_map"),hasWifiSignal:o?.wifi_signal!=null,hasRoomCoverage:o!=null&&"room_coverage"in o,hasDirtDensity:s!=null&&"dirt_density"in s,hasRobotSelectorHelper:!!e.robot_selector_helper&&!!t.states[e.robot_selector_helper]}}var G=class{constructor(n,e,o){this.hass=n;this.entryId=null;this.entityId=o??e.entity}updateHass(n){this.hass=n}async fetchSummary(n){let o=`/api/roomba_plus/${await this.resolveEntryId()}/mission_history?format=summary&days=${n}`,s=await this.hass.fetchWithAuth(o);if(!s.ok)throw new Error(`${s.status}`);return s.json()}async fetchRecords(n){let o=`/api/roomba_plus/${await this.resolveEntryId()}/mission_history?format=records&days=${n}`,s=await this.hass.fetchWithAuth(o);return s.ok?s.json():[]}async resolveEntryId(){if(this.entryId)return this.entryId;let n=await this.hass.callWS({type:"config/entity_registry/get",entity_id:this.entityId});return this.entryId=n.config_entry_id,this.entryId}};function v(t){return String(t??"").replace(/[&<>"']/g,n=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[n]??n)}function Y(t,n="en"){let e=Date.now()-new Date(t).getTime(),o=Math.floor(e/6e4);try{let s=new Intl.RelativeTimeFormat(n,{numeric:"auto"});if(o<1)return s.format(0,"minute");if(o<60)return s.format(-o,"minute");let r=Math.floor(o/60);if(r<24)return s.format(-r,"hour");let a=Math.floor(r/24);return a<30?s.format(-a,"day"):s.format(-Math.floor(a/30),"month")}catch{if(o<1)return"just now";if(o<60)return`${o}m ago`;let s=Math.floor(o/60);return s<24?`${s}h ago`:`${Math.floor(s/24)}d ago`}}var he={sofa:"\u{1F6CB}\uFE0F",bed:"\u{1F6CF}\uFE0F","bed-double":"\u{1F6CF}\uFE0F","silverware-fork-knife":"\u{1F37D}\uFE0F",stove:"\u{1F373}",microwave:"\u{1F4E6}",fridge:"\u{1F9CA}",toilet:"\u{1F6BD}",shower:"\u{1F6BF}",bathtub:"\u{1F6C1}",desk:"\u{1F5A5}\uFE0F","chair-rolling":"\u{1F4BA}",television:"\u{1F4FA}",bookshelf:"\u{1F4DA}",wardrobe:"\u{1F454}",home:"\u{1F3E0}",garage:"\u{1F697}",door:"\u{1F6AA}",stairs:"\u{1FA9C}",balcony:"\u{1F305}",pool:"\u{1F3CA}","washing-machine":"\u{1FAE7}",hanger:"\u{1F9F9}","baby-carriage":"\u{1F37C}",dog:"\u{1F415}",cat:"\u{1F408}","floor-plan":"\u{1F4D0}","map-marker":"\u{1F4CD}",star:"\u2B50",heart:"\u2764\uFE0F","office-building":"\u{1F3E2}",school:"\u{1F3EB}"},ge="\u{1F4CD}";var se={Auto:"Auto","\xD71":"One pass","\xD72":"Two passes"},fe={Auto:"Auto","One pass":"\xD71","Two passes":"\xD72"};function ae(t,n,e,o,s=!1){if(n.show_settings===!1)return"";let r=e,a=t.states[`switch.${r}_edge_clean`],i=t.states[`switch.${r}_always_finish`],l=t.states[`select.${r}_carpet_boost_mode`];if(!a&&!i&&!l)return"";let c="";if(o){let h=a?.state==="on",g=i?.state==="on",m=l?l.attributes.options??[]:[];c=`
    ...
    ${c}
  `}function ve(t){let{hass:n,config:e,caps:o,robotName:s,selectedRooms:r,passes:a,isSending:i,sendError:l,settingsPanelOpen:c}=t;if(!o.hasZones||e.show_rooms===!1)return"";let p=s,d=n.states[`select.${p}_smart_zone_select`],h=n.states[`select.${p}_zone_select`],g=d??h;if(!g)return"";let m=g.attributes.options??[];if(m.length===0)return"";let u=n.states[`button.${p}_repeat_mission`],b=!!u&&u.state!=="unavailable",C=n.states[`select.${p}_cleaning_passes`],S=o.isMop?"\u25B6 Mop selected rooms":"\u25B6 Clean selected rooms",f=r.size,x='<svg class="rpc-spinner" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" fill="none" stroke="currentColor" stroke-width="3" stroke-dasharray="31 63"/></svg>',$=(()=>{let _=o.hasSmartZones?`select.${s}_smart_zone_select`:`select.${s}_zone_select`,E=n.states[_]?.attributes?.region_icons;return E&&typeof E=="object"&&!Array.isArray(E)?E:{}})(),y=m.map(_=>{let E=r.has(_),q=$[_],V=q?he[q]??ge:"",j=V?`${V} ${v(_)}`:v(_);return`<button class="rpc-room-chip${E?" rpc-room-chip--selected":""}"
      data-room="${v(_)}" aria-pressed="${E}">${j}</button>`}).join(""),T="";if(C){let _=a;T=`
      <div class="rpc-passes-row">
        <span class="rpc-passes-label">Passes:</span>
        ${["Auto","\xD71","\xD72"].map(E=>`<button class="rpc-pass-chip${_===E?" rpc-pass-chip--selected":""}"
            data-pass="${E}"
            data-pass-option="${v(se[E]??E)}">${E}</button>`).join("")}
      </div>
    `}let A=ae(n,e,s,c);return`
  ...
  `}function Z(t,n){return t.states[n]?.state??"unavailable"}function be(t,n,e){return n==="m2"||n==="auto"&&e?`${Math.round(t*.0929)} m\xB2`:`${t} ft\xB2`}function Oe(t,n){if(!t)return null;for(let e=t.length-1;e>=0;e--){let o=t[e];if(o.missions&&o.missions.length>0)for(let s=o.missions.length-1;s>=0;s--){let r=o.missions[s];if(r.result==="completed")return Y(r.started_at,n)}else if(o.completed>0)return Y(o.date+"T12:00:00Z",n)}return null}function Fe(t){let n=["th","st","nd","rd"],e=t%100;return t+(n[(e-20)%10]??n[e]??n[0])}function ye(t){let{hass:n,config:e,caps:o,robotName:s,loadingAction:r,todayMissionCount:a,settingsPanelOpen:i}=t,l=e.entity,c=Z(n,l),p=n.states[l]?.attributes??{},d=n.config?.unit_system?.length==="m",h=e.area_unit??"auto",g=c==="unavailable",m=r!==null,u=s,b=`sensor.${u}_last_error_code`,C=`sensor.${u}_last_error_zone`,k=`sensor.${u}_mission_recharge_time`,S=`sensor.${u}_missions_last_30d`,f=`sensor.${u}_average_area_30d`,x=`sensor.${u}_area_cleaned_today`,$=p.mission_elapsed_min??null,y=p.mission_area_sqft??null,T=parseFloat(Z(n,S)),A=isNaN(T)||T<=0?45:T,_=parseFloat(Z(n,f)),E=o.isMop,q=E?"\u{1F9F9}":"\u{1F916}",V=v(p.friendly_name??l),j=n.states[`sensor.${u}_mission_phase`]?.state??"",Q=(n.states[`binary_sensor.${u}_mission_active`]?.state??"")==="on",J=o.hasMissionActive,P=n.states[`sensor.${u}_mission_expire_time`]?.state??"",B=P&&P!=="unavailable"&&P!=="unknown"?new Date(P):null,O=!!B&&!isNaN(B.getTime())&&B>new Date,U=O?Math.max(1,Math.round((B.getTime()-Date.now())/6e4)):null,L=!1;if(J)L=c==="docked"&&Q;else{let w=Z(n,k);L=c==="docked"&&(w!=="unavailable"&&w!=="unknown"&&P!=="unavailable"&&P!=="unknown")&&O}let N="",M="",te="";if(j==="evac")N="\u2B06",M="Emptying bin";else if(L)N="\u26A1",M=U!==null?`Recharging \u2014 resuming in ~${U} min`:"Recharging \u2014 mission continues";else switch(c){case"cleaning":N="\u25CF",M=E?"Mopping":"Cleaning";break;case"paused":N="\u23F8",M="Paused";break;case"returning":N="\u21A9",M="Returning to dock";break;case"docked":N="\u2713",M="Docked";break;case"idle":N="\u25CB",M="Idle";break;case"error":N="\u26A0",M="Error",te="rpc-error-state";break;case"unavailable":N="\u2014",M="Unavailable";break}let ce="";if(c==="error"){let w=n.states[b];if(w&&w.state!=="0"&&w.state!==""&&w.state!=="unavailable"){let R=v(w.attributes.description??"Unknown error"),z=v(w.attributes.action??""),H=Z(n,C),ee=H&&H!=="unknown"&&H!=="unavailable";M=`Error ${v(w.state)} \u2014 ${R}`,ce=`
        ${z?`<div class="rpc-error-action">${z}</div>`:""}
        ${ee?`<div class="rpc-error-zone">Zone: ${v(H)}</div>`:""}
      `}else M="Robot error \u2014 check the iRobot app"}let pe="";if((J?Q:c==="cleaning"||L)&&o.hasArea){let w=parseFloat(Z(n,x));if(!isNaN(w)&&w>0){let R=be(w,h,d),z=a!==null?a+1:null,H=z!==null&&z>1?` \xB7 ${v(Fe(z))} mission`:"";pe=`<div class="rpc-area-today">${R} already today${H}</div>`}}let de="";c==="cleaning"&&$!==null&&(de=`<div class="rpc-progress-track"><div class="rpc-progress-fill" style="width:${Math.min($/A*100,95)}%"></div></div>`);let ue="";if(c==="cleaning"){let w=[];if($!==null){let R=Math.max(0,Math.round(A-$));w.push(`<div class="rpc-metric"><span class="rpc-metric-val">~${R} min</span><span class="rpc-metric-lbl">Remaining</span></div>`)}if(o.hasArea&&y!==null&&(w.push(`<div class="rpc-metric"><span class="rpc-metric-val">${be(y,h,d)}</span><span class="rpc-metric-lbl">Cleaned</span></div>`),!isNaN(_)&&_>0)){let R=parseFloat(Z(n,`sensor.${u}_mission_count_30d`));if(!isNaN(R)&&R>=5){let z=Math.round((y-_)/_*100),H=z>=0?"\u25B2":"\u25BC",ee=z>=0?"rpc-delta-up":"rpc-delta-down";w.push(`<div class="rpc-metric"><span class="rpc-metric-val ${ee}">${H} ${Math.abs(z)}%</span><span class="rpc-metric-lbl">vs usual</span></div>`)}}w.length&&(ue=`<div class="rpc-metrics-row">${w.join("")}</div>`)}let X="";if(c==="docked"&&!L){let w=Oe(t.missionData,n.language);if(w)X=`<div class="rpc-docked-since">Last cleaned: ${w}</div>`;else{let R=n.states[l]?.last_changed;R&&(X=`<div class="rpc-docked-since">Last mission: ${Y(R,n.language)}</div>`)}if(o.hasDemandBlocked){let R=`binary_sensor.${s}_demand_clean_blocked`;n.states[R]?.state==="on"&&(X+='<div class="rpc-demand-blocked">\u{1F9F9} Floor needs cleaning \u2014 waiting for home to be empty</div>')}}let Ie='<svg class="rpc-spinner" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" fill="none" stroke="currentColor" stroke-width="3" stroke-dasharray="31 63"/></svg>',F=(w,R,z)=>{let H=r===w;return`<button class="rpc-btn${H?" rpc-btn-loading":""}"
      data-action="${w}"
      ${g||m?"disabled":""}
      aria-label="${R}">
      ${H?Ie:z}
    </button>`},W="";c==="cleaning"||j==="evac"?W=F("pause","Pause","\u23F8 Pause")+F("return_home","Return home","\u21A9 Return home"):c==="paused"?W=F("resume","Resume","\u25B6 Resume")+F("return_home","Return home","\u21A9 Return home"):L?W=F("return_home","Cancel mission","\u2715 Cancel mission"):c!=="returning"&&!g&&(W=F("start","Start","\u25B6 Start")+F("locate","Locate","\u2299 Locate"));let me=e.show_rooms!==!1;if(!me){let w=n.states[`button.${s}_repeat_mission`];!!w&&w.state!=="unavailable"&&(W+='<button class="rpc-btn-text" data-action="repeat-last">\u21A9 Repeat last</button>')}let Be=me?"":ae(n,e,s,i,!0);return`
  ...
  `}function _e(t,n){return Math.min(100,Math.max(0,Math.round(t/n*100)))}function xe(t,n){return n==="battery"?t>20?"var(--rpc-green)":t>10?"var(--rpc-amber)":"var(--rpc-red)":n==="tank"?t>40?"var(--rpc-green)":t>20?"var(--rpc-amber)":"var(--rpc-red)":t>50?"var(--rpc-green)":t>10?"var(--rpc-amber)":"var(--rpc-red)"}function We(t,n){let e=n/90;if(!e)return"";let o=t/e;return o>1.2?"\u2191":o<.8?"\u2193":"\u2192"}function $e(t){let n=parseInt(t,10);return!isNaN(n)&&n>=0?`~${n} use${n!==1?"s":""} remaining`:t==="Empty"?"Bag full \u2014 replace soon":t==="Full"?"Bag has capacity":v(t)}function we(t,n,e,o,s){if(n.show_health===!1)return"";let r=o,a=[];t.states[`sensor.${r}_filter_remaining_hours`]&&a.push({key:"filter",label:"Filter",sensorId:`sensor.${r}_filter_remaining_hours`,thresholdAttr:"threshold_hours",type:"consumable",wearSensorId:e.hasWearRate?`sensor.${r}_filter_wear_rate`:void 0,resetService:"reset_filter",lastReplacedId:`sensor.${r}_filter_last_replaced`}),e.hasBrush&&t.states[`sensor.${r}_brush_remaining_hours`]&&a.push({key:"brush",label:"Brush",sensorId:`sensor.${r}_brush_remaining_hours`,thresholdAttr:"threshold_hours",type:"consumable",wearSensorId:e.hasWearRate?`sensor.${r}_brush_wear_rate`:void 0,resetService:"reset_brush",lastReplacedId:`sensor.${r}_brush_last_replaced`}),e.hasPad&&t.states[`sensor.${r}_mop_pad_remaining_hours`]&&a.push({key:"pad",label:"Pad",sensorId:`sensor.${r}_mop_pad_remaining_hours`,thresholdAttr:"threshold_hours",type:"consumable",wearSensorId:e.hasWearRate?`sensor.${r}_pad_wear_rate`:void 0,resetService:"reset_pad",lastReplacedId:`sensor.${r}_pad_last_replaced`}),e.hasWater&&t.states[`sensor.${r}_mop_tank_level`]&&a.push({key:"tank",label:"Tank",sensorId:`sensor.${r}_mop_tank_level`,thresholdAttr:null,type:"tank"});let i=t.states[`sensor.${r}_battery_level`]?`sensor.${r}_battery_level`:t.states[`sensor.${r}_battery`]?`sensor.${r}_battery`:null,l=i?void 0:t.states[`vacuum.${r}`]?.attributes?.battery_level;if((i||l!==void 0)&&a.push({key:"battery",label:"Battery",sensorId:i??"",thresholdAttr:null,type:"battery",rawPct:l}),e.hasCleanBase&&t.states[`sensor.${r}_clean_base_status`]&&a.push({key:"cleanbase",label:"Clean Base",sensorId:`sensor.${r}_clean_base_status`,thresholdAttr:null,type:"cleanbase"}),a.length===0)return"";let c=a.map(m=>Ze(m,t,r,s)).join(""),p="";if(e.hasBatteryRetention){let m=t.states[`sensor.${r}_battery_capacity_retention`];if(m&&m.state!=="unavailable"&&m.state!=="unknown"){let u=Math.round(parseFloat(m.state));if(!isNaN(u)){let b=u>85?"var(--rpc-green)":u>70?"var(--rpc-amber)":"var(--rpc-red)",C=t.states[`sensor.${r}_charge_cycles`],k=C?parseInt(C.state,10):NaN,S=isNaN(k)?"":`${k} charge cycle${k!==1?"s":""}`,f="";if(e.hasBatteryEol){let y=t.states[`sensor.${r}_estimated_battery_eol`];if(y&&y.state!=="unavailable"&&y.state!=="unknown"){let T=parseInt(y.state,10);isNaN(T)||(f=T>0?`<div class="rpc-retention-eol">Battery life: ~${T} days remaining</div>`:'<div class="rpc-retention-eol rpc-retention-eol--warn">Consider replacing \u2014 battery at end of life</div>')}}let x=s.openPopover==="retention",$=x?`
          ...
          <div class="rpc-bar-row rpc-bar-row--static">
          ...
          </div>`;else{let C=Math.min(100,Math.round(parseFloat(m.state)));if(!isNaN(C)){let k=C>=85?"var(--rpc-green)":C>=65?"var(--rpc-amber)":"var(--rpc-red)",S=s.openPopover==="coverage",f=isNaN(b)?"":`Based on ${b} mission${b!==1?"s":""} in the last 30 days.`,x=S?`
            <div class="rpc-popover">
              <div class="rpc-popover-header">
            ...
            ${x}`}}}}let h=p||d?`<div class="rpc-health-battery-sep"></div>${p}${d}`:"",g="";if(e.isMop){let m=t.states[`sensor.${r}_mop_pad`],u=e.hasMopBehavior?t.states[`sensor.${r}_mop_behavior`]:null,b=[];m&&m.state!=="unknown"&&m.state!=="unavailable"&&b.push(v(m.state)),u&&u.state!=="unknown"&&u.state!=="unavailable"&&b.push(`${v(u.state)} intensity`),b.length&&(g=`
  ...
  `}function Ze(t,n,e,o){let s=o.openPopover===t.key;if(t.type==="cleanbase"){let h=n.states[t.sensorId];return h?`
    ...
    `:""}let r=0,a="",i="",l=null;if(t.rawPct!==void 0)r=Math.min(100,Math.max(0,t.rawPct)),a=`${Math.round(r)}%`;else{let h=n.states[t.sensorId];if(!h)return"";let g=parseFloat(h.state);if(isNaN(g))return"";if(t.type==="tank"||t.type==="battery")r=Math.min(100,Math.max(0,g)),a=`${Math.round(r)}%`;else{if(l=t.thresholdAttr?h.attributes[t.thresholdAttr]:null,!l)return"";r=_e(g,l),a=`${r}%`,i=`${Math.round(g)}h`}}let c=xe(r,t.type),p="";if(t.wearSensorId&&l){let h=n.states[t.wearSensorId];h&&h.state!=="unknown"&&h.state!=="unavailable"&&(p=We(parseFloat(h.state),l))}let d=t.rawPct!==void 0?{state:String(Math.round(t.rawPct)),attributes:{}}:n.states[t.sensorId];return`
    <div class="rpc-bar-row" data-bar="${t.key}" role="button" aria-expanded="${s}" tabindex="0"
    ...
    ${s&&d?qe(t,d,l,n,o):""}
  `}function qe(t,n,e,o,s){let r=parseFloat(n.state),a=e?_e(r,e):Math.min(100,Math.max(0,r)),i=xe(a,t.type),l=s.resetting===t.key,c=t.lastReplacedId?o.states[t.lastReplacedId]:null,p="";c&&c.state!=="unavailable"&&c.state!=="unknown"&&(p=`
  ...
  `}function je(t,n){if(!t||t==="unavailable"||t==="unknown")return"No schedule set";try{let e=new Date(t);return e.toLocaleDateString(n,{weekday:"short"})+" "+e.toLocaleTimeString(n,{hour:"2-digit",minute:"2-digit",hour12:!1})}catch{return v(t)}}function Ue(t,n){if(!t||t==="unavailable"||t==="unknown")return"";try{let e=new Date(t);if(isNaN(e.getTime()))return"";let o=e.toLocaleDateString(n,{weekday:"short"}),s=e.toLocaleTimeString(n,{hour:"2-digit",minute:"2-digit",hour12:!1});return`${o} ~${s}`}catch{return""}}function ke(t,n,e,o,s){if(n.show_schedule===!1)return"";let r=o,a=t.states[`sensor.${r}_next_clean`],i=t.states[`binary_sensor.${r}_schedule_hold_active`],l=t.states[`sensor.${r}_presence_clean_opportunities_7d`],c=t.states[`sensor.${r}_presence_clean_utilisation_7d`],p=t.states[`sensor.${r}_next_likely_clean_window`],d=!!l&&!!c&&l.state!=="unknown"&&l.state!=="unavailable"&&c.state!=="unknown"&&c.state!=="unavailable",h=!!p&&p.state!=="unknown"&&p.state!=="unavailable";if(!a&&!i&&!d&&!h)return"";let g="";if(i){let k=i.state==="on",f=i.attributes.source==="presence_manager",x="rpc-badge-green",$="Schedule active",y="";k&&(f?(x="rpc-badge-blue",$="Away hold",y="\u{1F3C3}"):(x="rpc-badge-amber",$="Hold active",y="\u{1F512}")),g=`
    ...
    `}let m="";if(h){let k=Ue(p.state,t.language);k&&(m=`
      ...
      `)}let u="",b=n.presence_entities??[];if(b.length>0){let k=b.map(S=>{let f=t.states[S];if(!f)return"";let x=f.state==="home",$=f.attributes.friendly_name??S,y=v($.split(" ")[0]);return`<span class="rpc-presence-dot">
        <span class="rpc-dot ${x?"rpc-dot-amber":"rpc-dot-green"}" aria-hidden="true"></span>
        ${y}
        <span class="rpc-presence-label">${x?"home":"away"}</span>
      </span>`}).join("");k&&(u=`<div class="rpc-presence-row">${k}</div>`)}let C="";if(d){let k=parseInt(l.state,10),S=parseInt(c.state,10);if(!isNaN(k)&&!isNaN(S)){let f=c.attributes.cleans_7d,x=f??Math.round(k*S/100),$=`${k} opportunit${k!==1?"ies":"y"} this week`;C=`
  ...
  `}function Ne(t,n){return n?`${Math.round(t*.0929)} m\xB2`:`${t} ft\xB2`}function Pe(t,n,e,o,s,r){if(n.show_history===!1)return"";let a=o,i=n.history_days??28,l=n.area_unit??"auto",c=l==="m2"||l==="auto"&&r,p=t.states[`sensor.${a}_clean_streak`],d=t.states[`sensor.${a}_completion_rate_30d`],h=p?parseInt(p.state,10):0,g=d?parseInt(d.state,10):NaN,m="",u=[];if(h>0&&u.push(`\u{1F525} ${h}-day streak`),isNaN(g)||u.push(`${g}% completion rate`),e.hasCleaningSpeedTrend){let x=t.states[`sensor.${a}_cleaning_speed_trend`]?.state;x==="declining"?u.push('<span class="rpc-trend-declining">\u2193 Speed declining</span>'):x==="improving"&&u.push('<span class="rpc-trend-improving">\u2191 Speed improving</span>')}u.length&&(m=`<div class="rpc-history-summary">${u.map((f,x)=>x===0?f:`<span class="rpc-summary-sep">\xB7</span>${f}`).join("")}</div>`);let b="";s.loading&&!s.data?b=He(Math.ceil(i/7)):s.error?b=`<div class="rpc-history-error">${v(s.error)}</div>`:s.data&&(b=Re(s.data,i,l,t.language),s.data.length<i&&(b+=`<div class="rpc-history-partial">Showing ${s.data.length} of ${i} days \u2014 full history builds over time</div>`));let C="";if(e.hasProblemZone){let f=t.states[`sensor.${a}_problem_zone`],x=t.states[`sensor.${a}_stuck_count_30d`];if(f&&f.state!=="unknown"&&f.state!=="unavailable"){let $=x?parseInt(x.state,10):0;$>0&&(C=`<div class="rpc-problem-zone">\u26A0 ${v(f.state)} \u2014 stuck ${$}\xD7 in 30 days</div>`)}}let k="";if(s.openDay){let x=new Date(s.openDay+"T00:00:00").toLocaleDateString(t.language,{weekday:"long",month:"long",day:"numeric",year:"numeric"}),$=s.dayMissions,y=s.openDaySummary,T="";if($===null)T="";else if(y&&y.total===0)T='<div class="rpc-day-empty">No missions this day</div>';else if($.length>0)T=$.map(_=>{let E=_.result==="completed"?"\u2713":"\u2717",q=_.result==="completed"?"rpc-day-ok":"rpc-day-err",V=new Date(_.started_at).toLocaleTimeString(t.language,{hour:"2-digit",minute:"2-digit",hour12:!1}),j=_.area_sqft!==null?Ne(_.area_sqft,c):"\u2014",le=_.zones?.map(O=>v(O)).join(" \xB7 ")??"",Q=n.show_dirt_events&&_.dirt_events!=null&&_.dirt_events>0?`${_.dirt_events} dirt event${_.dirt_events!==1?"s":""}`:"",J=[le,Q].filter(Boolean).join(" \xB7 "),P=_.initiator==="demand"?'<span class="rpc-initiator-badge">demand</span>':"",B="";if(_.wifi_signal&&_.wifi_signal.length>0){let O=Ae(_.wifi_signal),U=Math.min(...O),L=ze(O,U);B=`<div class="rpc-day-wifi" aria-label="Wi-Fi signal: minimum ${U}% during mission"><span aria-hidden="true">\u{1F4F6}</span>${L}<span>${U}% min</span></div>`}return`
          <div class="rpc-day-mission">
            ...
            ${B}
          </div>`}).join("");else if(y&&y.total>0){let _=y.area_sqft!==null?Ne(y.area_sqft,c):null;T=`
        <div class="rpc-day-aggregate">
          <div>${y.total} mission${y.total>1?"s":""} \xB7 ${v(y.result)}
            ${_?` \xB7 ${_} total`:""}</div>
          <div class="rpc-day-no-detail">Per-mission detail not available</div>
        </div>`}let A=y?.total??0;k=`
      <div class="rpc-popover rpc-day-popover">
        ...
        <div class="rpc-popover-divider"></div>
        ${A>0&&$&&$.length>0?`<div class="rpc-day-count">${A} mission${A>1?"s":""}</div>`:""}
        ${T}
      </div>
    `}let S="";if(n.show_lifetime!==!1){let f=t.states[`sensor.${a}_lifetime_missions`],x=t.states[`sensor.${a}_recent_area_30d`],$=t.states[`sensor.${a}_recent_time_30d`],y=f?parseInt(f.state,10):NaN,T=$?parseInt($.state,10):NaN,A=x?parseFloat(x.state):NaN;if(!isNaN(y)||!isNaN(T)||!isNaN(A)){let E=s.lifetimeExpanded?`
        <div class="rpc-lifetime-stats">
          <span class="rpc-lifetime-arrow">\u2192</span>
          ${isNaN(y)?"":`<span>${y.toLocaleString()} missions</span>`}
          ${isNaN(A)?"":`<span>${A.toLocaleString()} m\xB2</span>`}
          ${isNaN(T)?"":`<span>${T.toLocaleString()} h (30 d)</span>`}
        </div>`:"";S=`
